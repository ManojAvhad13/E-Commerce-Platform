        ...
        <header className='h-16 shadow-md bg-white fixed w-full z-40'>
            <div className='h-full container mx-auto flex items-center px-4 justify-between'>
                <div className=''>
                    <Link to={"/"}>
                        {/* <Logo w={90} h={50} /> */}
                        <img src="src\assets\Logo E-Comm.png" alt="logo" />
                    </Link>
                </div>
...

                        {
                            user?._id && (
                                <div className='text-3xl cursor-pointer relative flex justify-center' onClick={() => setMenuDisplay(preve => !preve)}>
                                    {
                                        user?.profilePic ? (
                                            <img src={user?.profilePic} alt="profilePic" className='w-10 h-10 rounded-full' />
                                        ) : (
                                            <FaRegUserCircle />
                                        )
                                    }
                    ...
                    <div>
                        {
                            user?._id ? (
                                ...
                                : (
                                    <Link to={"login"}>
                                        <button className='px-3 py-1 rounded-full bg-red-600 text-white hover:bg-red-700'>Login</button>
                                    </Link>
                                )}
